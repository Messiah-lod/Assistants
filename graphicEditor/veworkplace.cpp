...
//срабатывает по нажатию клавиши мышки
...
void VEWorkplace::mouseMoveEvent(QGraphicsSceneMouseEvent *event)
{
    ...
    case SelectionType: {//тупо рисуем синий прмоугольник
        if (m_leftMouseButtonPressed) {
            auto dx = event->scenePos().x() - m_previousPosition.x();
            auto dy = event->scenePos().y() - m_previousPosition.y();
...
void VEWorkplace::mouseReleaseEvent(QGraphicsSceneMouseEvent *event)
{
	if ((event->button() & Qt::LeftButton) ||
		...
		!(event->button() & Qt::MiddleButton))  m_leftMouseButtonPressed = false;
	...
	//после отжатия клавиш всегда выделение объектов
//	m_currentAction = SelectionType;
}
//сигнал двойного клика мышки
...
//отработка нажатий клавиатуры (дел и А)
...
//void VEWorkplace::wheelEvent(QGraphicsSceneWheelEvent *event)
//{
//	if (event->delta() > 0 || event->delta() < 0)
//	{
//		QMessageBox msExit;
//		msExit.setText(tr("wheelEvent!"));
//		msExit.exec();
//	}
//}