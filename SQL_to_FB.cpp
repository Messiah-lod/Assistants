...
{//После получения данных о базе и пользователе создаем подключение
	...
	if (isSucessConn)
	{
		logs << timeToLogs() << "Успешное подключение к БД." << std::endl;//Логин: " << Database->Username() << std::endl;
		return 0;
	}
...
void SQL_to_FB::write_blob(QAbstractItemModel* model)
{
...
// those parameters could have as well be omitted to simplify writing.
	IBPP::Transaction tr1 = IBPP::TransactionFactory(Database,
	...
	if (Database->Connected())
	{
		...
		if (tr1->Started())
		{
			logs << timeToLogs() << "Транзакция выполнена." << std::endl;
		}
		IBPP::Statement st1 = IBPP::StatementFactory(Database, tr1);
		IBPP::Blob bb = IBPP::BlobFactory(Database, tr1);
		
		std::string bbs = "1233";
		bb->Save(bbs);//сохранили в blob строку

		try
		{
			st1->Prepare("INSERT INTO PROJECT_TEST(ID, NUM_REV, OBJECT) VALUES (7, 1, ?)");//подготовили запрос с параметром
			if (st1->Parameters() != 1)//значение равно количеству знаков вопросов в запросе - параметров
			{
			//	//НЕуспешная запись
				logs << timeToLogs() << "Исключение в методе write_blob:" << std::endl;
				logs << timeToLogs() << "Определено неверное количество параметров для вставки!" << std::endl;
			}
			else
			{
				//если все хорошо
				st1->Set(1, bb);//определили параметр или параметры 1,2,3 и тд
				st1->Execute();//закомитили запрос
				//logs << timeToLogs() << "Запись blob выполнена" << std::endl;
			}
			st1->Close();
		}
		catch (std::exception& exp)//
...
//	" Имя запрошенной группы тех. программ: " << parentpage << ",размер: " << std::to_string(parentpage.size()) << std::endl;
	...
	for (int i = 0; i < name_page.length(); i++)
	{
		if (((name_page[i] >= 'а') && (name_page[i] <= 'я')) || //проверим кирилицу
			...
			(name_page[0] >= '0') && (name_page[0] <= '9')	 ||	//имя не начинается с цифры
			((name_page[i] >= '!') && (name_page[i] <= '/')) || //проверка отсутствия символов
	...
	if (std::any_cast<int>(parentID) == 0)
	{
		...
		for (int i = 0; i < name_page.length(); i++)
		{
			if (((parentpage[i] >= 'а') && (parentpage[i] <= 'я')) || //проверим кирилицу
				...
				std::any_cast<int>(tmpID) != 0 ||	//прверим уникальность имени
				(parentpage[0] >= '0') && (parentpage[0] <= '9') ||	//имя не начинается с цифры
				((parentpage[i] >= '!') && (parentpage[i] <= '/')) || //проверка отсутствия символов
...
//	std::vector <std::any> tmpListID;
	...
	for (size_t i = 0; i < objectIDlist.size(); i++)
	{
		...
		query.clear();
		query = "INSERT INTO ISAOBJFIELDS (ID, ISAOBJID, NAME, TID, DIRECTION)"
			"VALUES('" + any_to_str(tmpID) + "', '" + any_to_str(pageID) + "', 'HV_" + any_to_str(objectIDlist[i]) + "', '" + any_to_str(objectID) + "', 4)";
		write(query);
	}
	/*
	...
	std::string mark_obj, std::string name_obj, std::string evklid_obj, std::string disc_obj, std::string KKS_obj, std::string sign_obj)
{
	...
	if (KKS_obj.size() != 0)
	{
		...
		{
			logs << timeToLogs() << "Исключение в методе create_new_object:" << std::endl;
			logs << timeToLogs() << "KKS объекта уже существует! Объект не создан. Выполнение метода закончилось с кодом 4." << std::endl;
			return 3;
		}
	}
...
int SQL_to_FB::copy_page_with_object(std::string templatePage, std::string namePage, std::string parentPage, std::string idObject, std::string eventGroup)
{
	...
	idNewPage = idTemplatePage = pidPage = tmpID = 0;
	...
	//создадим аниматоры и рецепторы на новом кадре
		...
		create_new_anim_onpage("[flashfoncolor][event][Пред_2][1]", "0;128:128:128", namePage, "EVENT", eventGroup);
		create_new_anim_onpage("[flashfoncolor][event][пред][1]", "0;128:128:128", namePage, "EVENT", eventGroup);
		create_new_anim_onpage("[flashfoncolor][event][авар][1]", "0;128:128:128", namePage, "EVENT", eventGroup);
