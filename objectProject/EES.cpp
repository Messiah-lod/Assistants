	...
	try
	{
			...
			tableData->columnCount() > 1)// && tableData->rowCount() > 1)
		{
			...
			for (size_t j = 0; j < tableData->rowCount(); j++)//проверка присутвия тех. программы ДО проливки
			{
				...
				if (id_techprog != 1 || id_techprog == 0)
				{
					isTechProg.push_back(true);
				}
				else
				{
					isTechProg.push_back(false);
				}
			}
...
//				textEditLogs->append(("id new subject: " + std::to_string(id_subject)).c_str());
				dataBase.copy_page_with_object(mnemonicFrameTemplate, mnemonicFrameName, mnemonicFrameParent, std::to_string(id_subject), eventGroup);
				...
				//добавить условие проверки созданной тех. программы в данном цикле работы, если да, то создаем много объектов на одной, если нет, то не создаем объекты на программе
			...
			textEditLogs->append(tr("Unloading completed!", "txt_connect"));
		}
		else
		{
